subroutine Vortex_Panel(V_infinity,alpha,x_corner,z_corner,panel_count,Vpanel)
 implicit none
 integer*4 :: panel_count,iter1,iter2
 real*8    :: V_infinity, alpha, alpha_rad, A, B, C, D, E, F, G, P, Q, pi_number
 real*8, dimension(panel_count+1)    :: x_corner,z_corner
 real*8, dimension(1,panel_count+1)  :: Vinf_panel
 real*8, dimension(panel_count)      :: x_center,z_center,dx,dz,phi,S
 real*8, dimension(1,panel_count)    :: phi_mat, Vpanel
 real*8, dimension(panel_count+1,1)    :: gamma
 ...
 gamma =matmul(AN_inv,transpose(Vinf_panel))
    
 Vpanel =  V_infinity*cos(phi_mat - alpha_rad) + matmul(AT,gamma)
